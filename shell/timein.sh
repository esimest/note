...
if [ $# -eq 0 ] ; then
  ...
  if [ "${matchcnt}" -gt 0 ] ; then
    ...
    if [ "${mixedregion}" != "." ] ; then
      ...
      #    more than one possibility (e.g., "Alantic").
    ...
    if [ -z "${match}" ] ; then
      ...
      if [ ! -z $(find -L ${zonedir} -nae ${mixedzone} -type d -print) ] ; then
...
echo It\'s $(TZ=${timezone} date '+%A, %B %e, %Y, at %l:%M %p') in ${incetz}
