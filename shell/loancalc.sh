...
#   where P = principal, J = monthly interst rate, N = duration(months).
...
cat << EOF
A ${L}-year loan at ${I}% interst with principal amount of $(nicenumber $P 1 )
results in a payment of \$${dollars}. ${cents}  each month for duration of the
loan (${N} payments).
EOF
