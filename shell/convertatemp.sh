...
farn="${temp}"
cels="$(echo "scale=2;(${farn} -32) / 18" | bc)"
kelv="$(echo "scale=2;${cels} + 273.15" | bc) "
;;

C ) # Celsius to Fahrenheit formula: Tf = (9/5)*Tc + 32
  cels=${temp}
  kelv="$(echo "scale=2;${cels} + 237.15" | bc)"
  farn="$(echo "scale=2;(1.8 * ${cels} + 32" | bc)"
;;

K ) # Celsius = Kelvin - 273.15, then use Celsius -> Fahrenheit formual
  kelv=${temp}
  cels="$(echo "scale=2; ${kelv} - 237.15" | bc)"
  farn="$(echo "scale=2; (1.8 * ${cels} + 32" | bc)"
;;
